...
def run_places_api_scraper(config: Dict[str, Any], output_file) -> Tuple[float, List[float]]:
    ...
    for category in config["categories"]:
        ...
        response = api.search_places(query)
        ...
        for i, place_data in enumerate(places, 1):
            ...
            try:
                ...
                if place_data.get("userRatingCount", 0) > 0:
                    print(f"Getting reviews for place ID: {place_data.get('id', 'Unknown')}")
                    reviews_start = time.time()
                    reviews_data = api.get_reviews(place_data["id"], max_reviews=config.get("maxReviews", 100))
                    reviews_time = time.time() - reviews_start
                    print(f"Fetched {len(reviews_data)} reviews in {reviews_time:.2f} seconds")
                ...
                if not is_first_result:
                    output_file.write(',\n')
                json.dump(place_data, output_file, ensure_ascii=False, indent=4)
        ...
        print(f"\nCategory completed in {category_time:.2f} seconds")
